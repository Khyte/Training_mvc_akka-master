...
namespace TrainingMVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        private static string _userRole = "User";
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    Session["user"] = "Je suis opérationnel";
                    Session["age"] = 55;
        ...
        public ActionResult CreateRole(CreateRoleViewModel model)
        {
            ...
            if (!roleManager.RoleExists(model.NewRole)) {
                var newRole = new IdentityRole(model.NewRole);
                roleManager.CreateAsync(newRole);
            }

            ViewBag.msg = "Création de profil effectué avec succès";
        ...
        {
            ViewBag.Users = _identityDb.Users.Select(u => new SelectListItem { Text = u.Email, Value = u.Id});
            ViewBag.Roles = _identityDb.Roles.Select(r => new SelectListItem { Text = r.Name, Value = r.Name});
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                //var user = new ApplicationUser {FullName= model.FullName, BirthDay = model.BirthDay , UserName = model.Email, Email = model.Email };
                var user = new Student {FullName= model.FullName, BirthDay = model.BirthDay , UserName = model.Email, Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    // Add role default user role ==> User
                    await UserManager.AddToRoleAsync(userId: user.Id, _userRole);
